...
const LoginPage = ({ onSwitch, onSuccess }: any) => {
  ...
  return (
    <div className="max-w-md w-full relative">
      <div className="absolute -top-6 -left-2 bg-(--color-y2k-purple) text-(--color-y2k-button-text) px-4 py-1 font-black transform -rotate-1 border-2 border-(--color-y2k-solid-border) z-10 text-[10px] shadow-[2px_2px_0px_0px_var(--color-y2k-lime)]">
        SECURITY_ACCESS_v4.0
      </div>

      <Y2KCard className="relative pt-10">
        <div className="text-center mb-10">
          <div className="inline-block p-2 border-2 border-(--color-y2k-lime) mb-4 bg-(--color-y2k-bg-main)"><Car className="text-(--color-y2k-lime)" size={32} /></div>
          <h1 className="text-(--color-y2k-lime) text-5xl font-black italic tracking-tighter mb-1 leading-none">P-IOT</h1>
          <p className="text-(--color-y2k-purple) text-[10px] uppercase font-bold tracking-[0.4em]">Integrated Parking Node</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          <div className="space-y-2">
            <label className="flex items-center gap-2 text-(--color-y2k-lime) text-[10px] font-bold tracking-widest"><User size={12} /> User_Identification</label>
            <input type="text" placeholder="USER_ID" className="w-full bg-(--color-y2k-border) border-2 border-(--color-y2k-purple) p-3 text-(--color-y2k-text-main) focus:outline-none focus:border-(--color-y2k-lime) font-bold placeholder:opacity-20" value={credentials.username} onChange={(e) => setCredentials({ ...credentials, username: e.target.value })} required />
          </div>

          <div className="space-y-2">
            <label className="flex items-center gap-2 text-(--color-y2k-lime) text-[10px] font-bold tracking-widest"><Lock size={12} /> Access_Key</label>

"use client";
import React, { useState, useEffect, useRef, useMemo } from "react";
import { ParkingSession } from "@/types";
import { TrendCharts } from "@/components/analytics/TrendCharts";
import {
  BarChart3,
  TrendingUp,
  DollarSign,
  Clock,
  RefreshCw,
} from "lucide-react";

/**
 * AnalyticsTab
 * ------------
 * Fetches parking session data from /api/parking-sessions (server-side Azure Blob proxy).
 * Data is cached in a useRef so switching tabs back and forth costs ZERO extra fetches.
 * A manual Refresh button lets the user invalidate the cache on demand.
 *
 * ❌ REMOVED: getDocs(collection(db, "parking_sessions")) — was reading 2000+ Firestore docs
 *              on every single render, draining the daily quota in minutes.
 */
export default function AnalyticsPage() {
  const [sessions, setSessions] = useState<ParkingSession[]>([]);
  const [loading, setLoading] = useState(true);
  const [dataSource, setDataSource] = useState<string>("");

  // Cache — persists for the lifetime of the browser session (tab switches are free)
  const cache = useRef<ParkingSession[] | null>(null);

  const fetchSessions = async (forceRefresh = false) => {
    // Return cached data unless explicitly refreshed
    if (!forceRefresh && cache.current !== null) {
      setSessions(cache.current);
      setLoading(false);
      return;
    }

    setLoading(true);
    try {
      const response = await fetch("/api/parking-sessions");

      if (!response.ok) {
        throw new Error(`API responded with ${response.status}`);
      }

      // Show user where data came from (azure-blob or local-json-fallback)
      const source = response.headers.get("X-Data-Source") ?? "unknown";
      setDataSource(source);

      const data: ParkingSession[] = await response.json();
      cache.current = data;
      setSessions(data);
    } catch (error) {
      console.error("Gagal mengambil data sessions:", error);
    } finally {
      setLoading(false);
    }
  };

  // Fetch once on mount — subsequent tab visits served from cache
  useEffect(() => {
    fetchSessions();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // PROSES DATA (Logic Big Data)
  const processedData = useMemo(() => {
    // 1. Agregasi per Jam (Peak Hours)
    const hours = Array.from({ length: 24 }, (_, i) => ({
      hour: `${i.toString().padStart(2, "0")}:00`,
      count: 0,
    }));

    // 2. Agregasi per Hari (Revenue)
    const dailyRev: { [key: string]: number } = {};

    let totalRevenue = 0;
    let totalDuration = 0;

    sessions.forEach((s) => {
      const checkInDate = new Date(s.check_in);

      // Hitung jam sibuk
      const hr = checkInDate.getHours();
      hours[hr].count += 1;

      // Hitung revenue harian
      const dateKey = checkInDate.toISOString().split("T")[0]; // YYYY/MM/DD
      dailyRev[dateKey] = (dailyRev[dateKey] || 0) + s.fee;

      totalRevenue += s.fee;
      totalDuration += s.duration_minutes;
    });

    const revenueArray = Object.keys(dailyRev)
      .sort((a, b) => new Date(a).getTime() - new Date(b).getTime())
      .map((date) => ({
        date,
        total: dailyRev[date],
        label: new Date(date).toLocaleDateString("id-ID", {
          day: "2-digit",
          month: "short",
        }),
      }))
      .slice(-7); // Ambil 7 hari terakhir

    return {
      hourly: hours,
      daily: revenueArray,
      totalRevenue,
      avgDuration:
        sessions.length > 0 ? Math.round(totalDuration / sessions.length) : 0,
    };
  }, [sessions]);

  if (loading) {
    return (
      <div className="flex items-center justify-center py-20 text-[#C4FF4D] font-mono">
        <div className="animate-pulse">FETCHING_FROM_AZURE_BLOB...</div>
      </div>
    );
  }

  return (
    <div className="px-2 space-y-4 sm:space-y-6 md:space-y-8 animate-in fade-in duration-700">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:justify-between">
        <h1 className="text-[#C4FF4D] text-2xl sm:text-3xl font-black italic uppercase tracking-tighter flex items-center gap-3">
          <BarChart3 size={28} className="shrink-0" /> Big_Data_Analytics
        </h1>

        <div className="flex items-center gap-3 flex-wrap">
          {/* Data source badge */}
          <div
            className="px-3 py-1 text-[10px] font-black uppercase shadow-[3px_3px_0px_0px_#C4FF4D]"
            style={{
              backgroundColor:
                dataSource === "azure-blob" ? "#BA8CFF" : "#4D4D4D",
              color: "#1A1A1A",
            }}
          >
            {dataSource === "azure-blob" ? "⚡ Azure_Blob" : "📁 Local_JSON"}
          </div>

          <div className="bg-[#BA8CFF] text-[#1A1A1A] px-4 py-1 text-[10px] font-black uppercase shadow-[3px_3px_0px_0px_#C4FF4D]">
            Sample_Size: {sessions.length} Records
          </div>

          {/* Manual refresh button — invalidates client-side cache */}
          <button
            onClick={() => fetchSessions(true)}
            className="flex items-center gap-1 bg-[#1A1A1A] border-2 border-[#C4FF4D] text-[#C4FF4D] px-3 py-1 text-[10px] font-black uppercase hover:bg-[#C4FF4D] hover:text-[#1A1A1A] transition-colors active:scale-95"
            title="Force re-fetch from Azure"
          >
            <RefreshCw size={12} /> Refresh
          </button>
        </div>
      </div>

      {/* Ringkasan Statistik */}
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">
        <SummaryCard
          label="Total Revenue"
          value={`Rp ${processedData.totalRevenue.toLocaleString()}`}
          icon={<DollarSign size={20} />}
          color="#C4FF4D"
        />
        <SummaryCard
          label="Avg Duration"
          value={`${processedData.avgDuration} Mins`}
          icon={<Clock size={20} />}
          color="#BA8CFF"
        />
        <SummaryCard
          label="Completion Rate"
          value={
            sessions.length > 0
              ? `${Math.round(
                  (sessions.filter((s) => s.status === "completed").length /
                    sessions.length) *
                    100,
                )}%`
              : "0%"
          }
          icon={<TrendingUp size={20} />}
          color="#C4FF4D"
        />
      </div>

      {/* Grafik */}
      <TrendCharts
        hourlyData={processedData.hourly}
        dailyRevenue={processedData.daily}
      />
    </div>
  );
}

const SummaryCard = ({
  label,
  value,
  icon,
  color,
}: {
  label: string;
  value: string;
  icon: React.ReactNode;
  color: string;
}) => (
  <div
    className="bg-[#1A1A1A] border-4 p-4 sm:p-6 shadow-[5px_5px_0px_0px_#000]"
    style={{ borderColor: color }}
  >
    <div className="flex justify-between items-center opacity-70 mb-2">
      <span
        className="text-[10px] font-black uppercase tracking-[0.2em]"
        style={{ color }}
      >
        {label}
      </span>
      <div style={{ color }}>{icon}</div>
    </div>
    <div className="text-2xl sm:text-3xl font-black italic break-all">
      {value}
    </div>
  </div>
);

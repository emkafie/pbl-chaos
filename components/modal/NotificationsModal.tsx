...
function OperatorNotificationItem({
  ...
  return (
    ...
    >
      ...
      {!notif.isRead && (
        <div className="absolute top-0 left-0 w-1 h-full bg-(--color-y2k-purple)" />
      )}
      ...
      <div className="flex items-center justify-between mt-3">
        ...
        {!notif.isRead ? (
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 bg-(--color-y2k-lime) text-(--color-y2k-button-text) border border-(--color-y2k-solid-border) text-[9px] font-black uppercase tracking-wider hover:shadow-[2px_2px_0px_0px_var(--color-y2k-purple)] hover:translate-x-0.5 hover:translate-y-0.5 transition-all active:scale-95"
          >
    ...
    <div className="fixed inset-0 z-100 flex items-center justify-center p-4 md:p-6">
      ...
      >
              ...
              {loading ? (
                <div className="flex items-center justify-center py-12">
                  <div className="flex items-center gap-3">
                    <div className="w-2 h-2 bg-(--color-y2k-purple) animate-pulse" />
                ...
                <div className="flex flex-col items-center justify-center py-12 gap-3">
                  <CheckCircle2
                    size={32}
                    className="text-(--color-y2k-lime) opacity-50"
                  />
                  <span className="text-[11px] text-gray-500 font-bold uppercase tracking-widest">
                    NO_ALERTS_IN_QUEUE
                  </span>
                  ...
                  {latestNotif && (
                    <div>
                      <div className="flex items-center gap-2 mb-2">
                        <div className="w-1.5 h-1.5 bg-(--color-y2k-lime) animate-pulse" />
                        <span className="text-[9px] text-(--color-y2k-lime) font-black uppercase tracking-[0.2em]">
                          LATEST_ALERT
                        </span>
                      </div>
                  ...
                  {notifications.length > 1 && (
                    <div className="flex flex-col gap-2 overflow-hidden">
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex-1 h-px bg-(--color-y2k-border)" />
            ...
            <div className="flex flex-col gap-5 overflow-hidden">
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2 mb-1">
                  <Send size={12} className="text-(--color-y2k-lime)" />
                  <span className="text-[9px] text-(--color-y2k-lime) font-black uppercase tracking-[0.2em]">
                    COMPOSE_BROADCAST
                  </span>
                </div>
                ...
                <textarea
                  ...
                  maxLength={500}
                  rows={3}
                  className="w-full bg-[#0D0D0D] border-2 border-(--color-y2k-border) text-(--color-y2k-text-main) text-xs font-bold uppercase tracking-tight p-3 resize-none focus:outline-none focus:border-(--color-y2k-lime) transition-colors placeholder:text-gray-600 placeholder:italic"
                />
                      ...
                      <button
                        ...
                        className={`flex items-center gap-1.5 px-3 py-1.5 border-2 text-[9px] font-black uppercase tracking-wider transition-all ${
                          isActive
                            ...
                            : "border-(--color-y2k-border) text-gray-500 hover:text-gray-300 hover:border-gray-400"
                        }`}
                      >
                        {cfg.icon}
                        {cfg.label}
                ...
                <button
                  ...
                  className={`w-full flex items-center justify-center gap-2 py-3 border-2 border-(--color-y2k-solid-border) font-black uppercase text-xs tracking-widest transition-all ${
                    !message.trim() || sending
                      ? "bg-(--color-y2k-border)/30 text-gray-600 cursor-not-allowed"
                      : "bg-(--color-y2k-lime) text-(--color-y2k-button-text) shadow-[4px_4px_0px_0px_var(--color-y2k-purple)] hover:shadow-none hover:translate-x-1 hover:translate-y-1 active:scale-[0.98]"
                  }`}
                >
                  {sending ? (
                    <>
        ...
        <button
          ...
          className="absolute -top-3 -right-3 bg-(--color-y2k-purple) text-(--color-y2k-button-text) p-2 border-2 border-(--color-y2k-solid-border) shadow-[2px_2px_0px_0px_var(--color-y2k-lime)] hover:scale-110 active:scale-95 transition-all z-10"
        >

          ...
          <div className="space-y-4">
            {visibleLogs.length > 0 ? (
              visibleLogs.map((log, i) => (
                <div key={i} className="flex justify-between items-center border-b border-(--color-y2k-border) pb-2">
                  <div>
                    <p className="text-xs font-black">{log.name}</p>
                    <p className="text-[8px] text-(--color-y2k-purple) font-bold uppercase">{log.role}</p>
                  </div>
                  <span className="text-[10px] text-gray-500 font-bold italic">{log.time}</span>
                </div>
              ))
            ) : (
              <div className="text-[10px] text-(--color-y2k-text-muted) uppercase tracking-widest">
            ...
            {recentLogins.length > 3 && (
              <button
                ...
                className="w-full text-[10px] font-black text-(--color-y2k-lime) underline uppercase mt-2"
              >
